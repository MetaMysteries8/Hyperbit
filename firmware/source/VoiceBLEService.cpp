...
static const uint8_t HB_BASE_UUID[16] = {
    0x70,0x1f,0x8b,0x5e,0x2d,0xc6,0x31,0x9a,
    0x8f,0x4b,0x1d,0x4c,0x00,0x00,0x9a,0x7f
};

static const uint16_t HB_SERVICE_UUID = 0x0001;
static const uint16_t HB_CHAR_UUID[HB_CHAR_COUNT] = {0x0002, 0x0003, 0x0004};

VoiceBLEService::VoiceBLEService() : ttsReceiving(false), ttsReadyFlag(false), ttsLen(0), expectedSpeakerSeq(0), pcStateValue(0) {
...
void VoiceBLEService::onDataWritten(const microbit_ble_evt_write_t *params) {
    if (params->handle == valueHandle(HB_SPEAKER)) {
        if (!ttsReceiving || params->len < 2) return;
        ...
        for (int i = 0; i < payload; i++) TTS_BUFFER[ttsLen++] = params->data[i + 1];
        return;
    }
